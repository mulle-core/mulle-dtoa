...
auto umul192_upper64_modified(
    uint64_t pow10_hi, uint64_t pow10_lo, uint64_t scaled_sig) -> uint64_t {
...
void write(char* buffer, uint64_t dec_sig, int dec_exp) noexcept {
  int len = floor_log10_pow2(
    std::numeric_limits<uint64_t>::digits - std::countl_zero(dec_sig));
...
void dtoa_schubfach(char* buffer, double x) {
